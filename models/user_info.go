...
var (
	userInfoDAO  *UserInfoDAO //用户信息对象
	userInfoOnce sync.Once    //行为只会被执行一次
)

func NewUserInfoDAO() *UserInfoDAO { //新建
...
func (u *UserInfoDAO) QueryUserInfoById(userId int64, userinfo *UserInfo) error { //查询用户信息
	if userinfo == nil {
		return ErrIvdPtr
	}
...
func (u *UserInfoDAO) AddUserInfo(userinfo *UserInfo) error { //添加用户信息
	if userinfo == nil {
		return ErrIvdPtr
	}
...
func (u *UserInfoDAO) IsUserExistById(id int64) bool { //用户是否退出
...
func (u *UserInfoDAO) AddUserFollow(userId, userToId int64) error { //用户关注
	return DB.Transaction(func(tx *gorm.DB) error { //gorm事务处理
...
func (u *UserInfoDAO) CancelUserFollow(userId, userToId int64) error { //取消关注
...
func (u *UserInfoDAO) GetFollowListByUserId(userId int64, userList *[]*UserInfo) error { //用户关系
	if userList == nil {
		return ErrIvdPtr
	}
	var err error
